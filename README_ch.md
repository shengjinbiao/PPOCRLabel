...
#### 近期更新
- 2025.11:
  - 新增`PaddleOCR 检测模型训练`入口，可在菜单中一键启动训练、实时查看日志/停止任务，并支持自动缓存训练配置。
  - 支持从当前打开并已完成标注的目录自动拆分训练/验证数据集，省去手动复制 `train.txt`/`val.txt` 的步骤。
  - “自动标注当前页”：按钮增加了一个下拉选项，只标注当前页面，而不是整个目录。
- 2025.06:
